	...
	pub fn insert(&mut self, image_rectangle: rect_structs::RectWH) -> Option<RectT> {
		for candidate_space in self.spaces.iter().rev() {
			let res = if <RectT>::ALLOW_FLIP && self.allow_flipping {
				let normal = insert_and_split::insert_and_split(image_rectangle.get_wh(), candidate_space);
				let flipped = insert_and_split::insert_and_split(image_rectangle.get_wh().flip(), candidate_space);
