...
impl DebuggerState {
	...
	pub fn checkpoint(&mut self, name: &'static str) {
		self.checkpoint_data.push(self.checkpoint.elapsed().as_secs_f32());
...
impl egui_util::EguiComponent for DebuggerState {
	...
	fn render(&mut self, context: egui::CtxRef, delta_time: f64) {
		self.profiler_data.push_front((delta_time as f32, self.p_checkpoint_data.clone()));
		egui::Window::new("debug").show(&context, |ui| {
			egui::CollapsingHeader::new("Frame Times").show(ui, |ui| {
				let mut sum = 0.0;
				let items: Vec<_> = self.profiler_data.iter().enumerate().map(|(i, v)| {
					sum += v.0;
					(i, (sum, v.1.clone(), v.0))
				}).take_while(|(_, (v, _, _))| *v <= self.sample_time).collect();
				let mut lines = vec![];
				...
				ui.add(egui::Slider::new(&mut self.sample_time, 0.1..=100.0)
					.text("Sample time")
					.suffix(" s")
					.logarithmic(true)
				...
				ui.label(format!("Frame time of {:.3}ms ({:.3}fps)", 1000.0 / framerate, framerate));
				let mut prev_time = 0.0;
				for ((_, i), j) in lines.iter().zip(self.p_checkpoint_labels.iter()) {
					ui.label(format!("\t•  {} took {:.3}ms", j, (*i - prev_time) * 1000.0));
					prev_time = *i;
				}
				ui.label(format!("\t•  free time of {:.3}ms", (prev_time - sum / self.profiler_data.len() as f32) * 1000.0));
				ui.label(format!("{} samples", self.profiler_data.len()));
				egui::CollapsingHeader::new("Plot").show(ui, |ui| {
					let plot_factor = if self.sample_time - real_time < 0.1 {self.sample_time / real_time} else {1.0};
						...
						.show(ui, |plot| {
							for (i, _) in lines {
								plot.line(plot::Line::new(plot::Values::from_values_iter(
									i.iter().map(|(j, v)| plot::Value::new(-(plot_factor * *j) as f64, 1000.0 * *v as f64)))));
							}
							plot.line(plot::Line::new(plot::Values::from_values_iter(
								items.iter().map(|(_, (j, _, v))| plot::Value::new(-(plot_factor * *j) as f64, 1000.0 * *v as f64)))));
					});
				});
			});
			egui::CollapsingHeader::new("Fun Debug Modifiers").show(ui, |ui| {
				self.modifiers_changed |= ui.add(egui::Slider::new(&mut self.dm_screen_offset[0], 0.0..=1.0)
					.text("state.screen_offset[0]")).changed();
				self.modifiers_changed |= ui.add(egui::Slider::new(&mut self.dm_screen_offset[1], 0.0..=1.0)
					.text("state.screen_offset[1]")).changed();
			});
